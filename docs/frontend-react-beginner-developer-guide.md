...
Why this comes first: without `index.html`, the browser has no page to open.
...
Purpose: connect React to `index.html`.
...
Line 3 imports Node URL helper.

Line 5 calculates the repository root directory.

Lines 7-30 export Vite configuration.

Line 8 loads `.env` values from the root project.

Line 9 reads `BACKEND_URL` or defaults to `http://localhost:3000`.

Line 12 tells Vite the frontend root is the repository root.

Line 13 enables React support.

Line 14 tells Vite where public assets are.

Lines 15-17 set build output to `dist/frontend`.

Lines 18-28 configure the dev server.

Line 19 reads frontend host from env or uses `localhost`.

Line 20 reads frontend port from env or uses `5173`.

Lines 21-27 configure the `/api` proxy.
...
The important frontend connection is in `index.html`:
